...
def run_main_menu(
    ...
    while True:
        ...
        elif choice == "4": _menu_reports(ui, students, courses, exports)
        elif choice == "0":
...
def _menu_reports(
    ...
    while True:
        ...
        ui.console.print(
            ...
            "[bold]4.[/bold] Export reports to CSV\n"
            "[bold]5.[/bold] Export reports to JSON\n"
            "[bold]0.[/bold] Back"
        )
        choice = ui.ask_choice("Select an option", ["0", "1", "2", "3", "4", "5"])
        if   choice == "1": _report_top_students(ui, students)
            ...
            render_courses_table(ui.console, courses.all(), courses)
            ui.pause()
        elif choice == "4": _action_export(ui, exports, fmt="csv")
        elif choice == "5": _action_export(ui, exports, fmt="json")
        elif choice == "0": return
...
@catch_errors
def _action_export(ui: ConsoleUI, exports: ExportService, *, fmt: str) -> None:
    """Run a CSV or JSON export with a rich progress spinner."""
    label = fmt.upper()
    with Progress(
        SpinnerColumn(),
        TextColumn(f"[bold cyan]Exporting reports to {label}…[/bold cyan]"),
        transient=True,
        console=ui.console,
    ) as progress:
        progress.add_task("export", total=None)
        # Tiny pause so the spinner is actually visible for very small datasets.
        time.sleep(0.4)
        result: ExportResult = (
            exports.export_csv() if fmt == "csv" else exports.export_json()
        )

    ui.success(f"{label} export complete. Files saved to:")
    ui.console.print(f"  [bold]{result.folder}[/bold]")
    for path in result.files:
        ui.console.print(f"   • {path.name}")
    ui.pause()
